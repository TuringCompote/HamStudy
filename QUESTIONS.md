...
| # | Question | Decision |
|---|----------|----------|
| 18 | Explanation generation | **Batch-precompute** a deep "why" explanation for every question via the Message Batches API (50% off) + prompt caching, stored in `notes`/`explanations` by `bank_version`. `explain()` = cache-first DB lookup. Live AI reserved for `diagnose()` + `narrate()`. Regenerate only on bank revision. (Spec §6f.) |
